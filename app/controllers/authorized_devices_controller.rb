class AuthorizedDevicesController < ApplicationController
	...
	def update
		if @authorized_device.update(authorized_device_params)
			redirect_to authorized_devices_path, notice: t('controller.authorized_device.update.success')
		else
			redirect_back fallback_location: root_path,
										notice: t('controller.fail', @authorized_device.errors.messages[:description].first)
		end
	end

	def destroy
		if @authorized_device.destroy
			redirect_to authorized_devices_path, notice: t('controller.authorized_device.desroy.success')
		else
			redirect_back fallback_location: root_path,
										notice: t('controller.fail', @authorized_device.errors.messages[:description].first)
		end
